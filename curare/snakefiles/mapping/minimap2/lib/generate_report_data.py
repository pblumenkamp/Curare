"""
Convert minimap2 results to usable data for the large report

Usage:
    generate_report_data.py --stats <minimap2_stats> --settings <settings> --output <output> [--paired-end]
...
Options:
    ...
    -s <minimap2_stats> --stats <minimap2_stats>                 TSV containing the statistics of all minimap2 runs
    -t <settings> --settings <settings>                        All minimap2 settings as an yaml file
    -o <output> --output <output>                              Created js containing minimap2 statistics
...
def create_minimap2_stats_js_object(stats_file: Path) -> Dict[str, Any]:
    tsv_df = pd.read_csv(stats_file, sep="\t")
    return json.loads(tsv_df.to_json(orient='records'))

def create_minimap2_settings_js_object(settings_file: Path) -> Dict[str, Any]:
...
def generate_report_data(output_file: Path, stats_file: Path, settings_file: Path, is_paired_end: bool):
    ...
    with output_file.open('w') as f:
        f.write('window.Curare.minimap2 = (function() {\n')
