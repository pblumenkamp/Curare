"""
Convert bowtie results to usable data for the large report

Usage:
    generate_report_data.py --stats <bowtie_stats> --settings <settings> --output <output> [--paired-end]
...
Options:
    ...
    -s <bowtie_stats> --stats <bowtie_stats>                 TSV containing the statistics of all bowtie runs
    -t <settings> --settings <settings>                        All bowtie settings as an yaml file
    -o <output> --output <output>                              Created js containing bowtie statistics
...
def create_bowtie_stats_js_object(stats_file: Path) -> Dict[str, Any]:
    tsv_df = pd.read_csv(stats_file, sep="\t")
    return json.loads(tsv_df.to_json(orient='records'))

def create_bowtie_settings_js_object(settings_file: Path) -> Dict[str, Any]:
...
def generate_report_data(output_file: Path, stats_file: Path, settings_file: Path, is_paired_end: bool):
    stats = create_bowtie_stats_js_object(stats_file)
    settings = create_bowtie_settings_js_object(settings_file)

    with output_file.open('w') as f:
        f.write('window.Curare.bowtie = (function() {\n')
