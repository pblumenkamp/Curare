...
Options:
    ...
    -t <settings> --settings <settings>                        All STAR settings as an yaml file
...
def create_star_stats_js_object(stats_file: Path) -> Dict[str, Any]:
    tsv_df = pd.read_csv(stats_file, sep="\t")
    return json.loads(tsv_df.to_json(orient='records'))

def create_star_settings_js_object(settings_file: Path) -> Dict[str, Any]:
...
def generate_report_data(output_file: Path, stats_file: Path, settings_file: Path, is_paired_end: bool):
    stats = create_star_stats_js_object(stats_file)
    settings = create_star_settings_js_object(settings_file)

    with output_file.open('w') as f:
        f.write('window.Curare.star = (function() {\n')
