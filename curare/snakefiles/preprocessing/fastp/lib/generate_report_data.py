...
def create_fastp_stats_js_object(reports_path: Path, is_paired_end: bool) -> List[Dict[str, str]]:
    report_list = []
...
def generate_report_data(output_file: Path, reports_file: Path, is_paired_end: bool):
    reports = create_fastp_stats_js_object(reports_file, is_paired_end)

    with output_file.open('w') as f:
        f.write('window.Curare.fastp = (function() {\n')
